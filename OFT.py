...
for iteration in range(10):
    ...
    for data_index in [0,1,2,3,4,5,6]:
        ...
        config = OFTConfig(
            r=ranks[data_index],
            target_modules=['conv1','conv2','conv3','fc'],
            module_dropout=0.0,
            init_weights=True
        )
        OFT_model = OFTModel(pretrained_net, config,'default')
        #########

        OFT_model = OFT_model.to(device)
        lr,num_epochs = 0.01,110

        optimizer = optim.SGD(filter(lambda p: p.requires_grad, OFT_model.parameters()), momentum=0.9, lr=0.01, weight_decay=0.0005)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for images, labels in tqdm(train_loader):
                log_train = train_one_batch_finetune(images, labels, OFT_model)
            ...
            lr_scheduler1.step()
            OFT_model.eval()
            log_test = evaluate_testset(OFT_model)
